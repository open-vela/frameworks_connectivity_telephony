...
#define EVENT_UNREGISTER_STK_AGENT_DONE 0x93
...
static int tapi_open_handler(char* dbus_name, unsigned int spec_service)
{
    if (g_context != NULL) {
    ...
    if (spec_service == TAPI_SERVICE_NONE || spec_service == TAPI_SERVICE_FULL)
        g_context = tapi_open(dbus_name, on_tapi_client_ready, NULL);
    else
        g_context = tapi_open_service(dbus_name, on_tapi_client_ready, NULL, spec_service);
...
static int tapi_close_handler(void)
{
    if (g_context == NULL) {
...
static void tele_stk_async_func(tapi_async_result* result)
{
    ...
    if (result->status != OK) {
        syslog(LOG_DEBUG, "%s msg id: %d result err, return.", __func__, result->msg_id);
        return;
    }

    if (result->msg_id == EVENT_REGISTER_STK_AGENT_DONE) {
        syslog(LOG_DEBUG, "register stk agent (%s) done", (char*)result->data);
    } else if (result->msg_id == EVENT_UNREGISTER_STK_AGENT_DONE) {
        syslog(LOG_DEBUG, "unregister stk agent (%s) done", (char*)result->data);
    }
}
...
static int telephonytool_cmd_unregister_stk_agent(tapi_context context, char* pargs)
{
    char dst[2][MAX_INPUT_ARGS_LEN];
    char* slot_id;
    char* agent_id;
    ...
    syslog(LOG_DEBUG, "%s, slot_id: %s, agent_id: %s", __func__, slot_id, agent_id);
    return tapi_stk_agent_unregister(context, atoi(slot_id),
        EVENT_UNREGISTER_STK_AGENT_DONE, agent_id, tele_stk_async_func);
}
...
static int telephonytool_cmd_get_main_menu_title(tapi_context context, char* pargs)
{
    char dst[1][MAX_INPUT_ARGS_LEN];
    char* slot_id;
    char* title;
...
static int telephonytool_cmd_get_imei(tapi_context context, char* pargs)
{
    ...
    tapi_get_imei(context, atoi(slot_id), imei, sizeof(imei));
...
{
    char* slot_id;
    char imeisv[MAX_IMEISV_LENGTH + 1] = { 0 };
    ...
    tapi_get_imeisv(context, atoi(slot_id), imeisv, sizeof(imeisv));
...
static int telephonytool_cmd_get_phone_number(tapi_context context, char* pargs)
{
    ...
    tapi_get_phone_number(context, atoi(slot_id), number, sizeof(number));
...
static int telephonytool_cmd_get_sim_iccid(tapi_context context, char* pargs)
{
    ...
    tapi_sim_get_sim_iccid(context, atoi(slot_id), iccid, sizeof(iccid));
...
static int telephonytool_cmd_get_sim_operator_name(tapi_context context, char* pargs)
{
    ...
    tapi_sim_get_sim_operator_name(context, atoi(slot_id), spn, sizeof(spn));
...
{
    char* slot_id;
    char smsc_addr[MAX_CENTER_ADDRESS_LENGTH + 1] = { 0 };
    ...
    tapi_sms_get_service_center_address(context, atoi(slot_id), smsc_addr, sizeof(smsc_addr));
...
{
    char* slot_id;
    char _operator[MAX_NETWORK_OPERATOR_NAME_LENGTH + 1] = { 0 };
    ...
    tapi_network_get_display_name(context, atoi(slot_id), _operator, sizeof(_operator));
...
{
    char network_operator_name[MAX_NETWORK_OPERATOR_NAME_LENGTH + 1] = { 0 };
    char* slot_id;
    int ret = -1;
    ...
    ret = tapi_network_get_operator_name(context, atoi(slot_id),
        network_operator_name,
        sizeof(network_operator_name));
...
static int telephonytool_cmd_open(tapi_context context, char* pargs)
{
    ...
    tapi_open_handler(dbus_name, spec_service_id);
...
static struct telephonytool_cmd_s g_telephonytool_cmds[] = {
    ...
    { "answer_0", CALL_CMD,
        ...
        "answer (enter example : answer_0 0 [call_id] /ril_0/voicecall01)" },
    { "hangup_0", CALL_CMD,
        telephonytool_cmd_hangup_by_id,
        "hangup (enter example : hangup_0 0 [call_id] /ril_0/voicecall01)" },
    ...
    { "separate", CALL_CMD,
        ...
        "call separate  (enter example : separate 0 [slot_id][call_id: /ril_0/voicecall01])" },
    ...
    { "register-stk-agent", STK_CMD,
        ...
        "[slot_id][agent_id])" },
    { "unregister-stk-agent", STK_CMD,
        telephonytool_cmd_unregister_stk_agent,
        "unregister stk agent (enter example : unregister-stk-agent 0 /stk_agent_default_0 "
        "[slot_id][agent_id])" },
...
bool execute_telephony_cmd(char* cmd, char* arg)
{
    ...
    for (i = 0; g_telephonytool_cmds[i].cmd; i++) {
        if (strcmp(cmd, g_telephonytool_cmds[i].cmd) == 0) {
            find_flag = true;
            if (g_telephonytool_cmds[i].pfunc(g_context, arg) < 0)
