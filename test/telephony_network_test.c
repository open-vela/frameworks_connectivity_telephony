...
int tapi_net_select_auto_test(int slot_id)
{
    ...
    int ret = tapi_network_select_auto(get_tapi_ctx(), slot_id, EVENT_REGISTER_AUTO_DONE,
        network_event_callback);
    if (ret) {
        syslog(LOG_ERR, "tapi_network_select_auto execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
    ...
    if (judge()) {
        syslog(LOG_ERR, "network_event_callback is not executed in %s", __func__);
        res = -1;
        goto on_exit;
    }
...
int tapi_net_select_manual_test(int slot_id, char* mcc, char* mnc, char* tech)
{
    ...
    int ret = tapi_network_select_manual(get_tapi_ctx(), slot_id, EVENT_REGISTER_MANUAL_DONE,
        network_info, network_event_callback);
    free(network_info);
    if (ret) {
        syslog(LOG_ERR, "tapi_network_select_manual execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
...
int tapi_net_scan_test(int slot_id)
{
    ...
    int ret = tapi_network_scan(get_tapi_ctx(), slot_id, EVENT_NETWORK_SCAN_DONE,
        network_event_callback);
    if (ret) {
...
int tapi_net_registration_info_test(int slot_id)
{
    ...
    int ret = tapi_network_get_registration_info(get_tapi_ctx(), slot_id,
        EVENT_QUERY_REGISTRATION_INFO_DONE, network_event_callback);
    if (ret) {
        syslog(LOG_ERR, "tapi_network_get_registration_info execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
...
int tapi_net_get_serving_cellinfos_test(int slot_id)
{
    ...
    global_data.serving_cell_reg = -1;
    int ret = tapi_network_get_serving_cellinfos(get_tapi_ctx(), slot_id,
        EVENT_QUERY_SERVING_CELL_DONE, network_event_callback);
    if (ret) {
        syslog(LOG_ERR, "tapi_network_get_serving_cellinfos execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
...
int tapi_net_get_neighbouring_cellInfos_test(int slot_id)
{
    ...
    int ret = tapi_network_get_neighbouring_cellinfos(get_tapi_ctx(), slot_id,
        EVENT_QUERY_NEIGHBOURING_CELL_DONE, network_event_callback);
    if (ret) {
        syslog(LOG_ERR, "tapi_network_get_neighbouring_cellinfos execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
...
int tapi_net_query_signalstrength_test(int slot_id)
{
    ...
    int ret = tapi_network_get_signalstrength(get_tapi_ctx(), slot_id, &ss);
...
int tapi_net_get_voice_registered_test(int slot_id)
{
    ...
    syslog(LOG_DEBUG, "%s, slot_id: %d, voice_reg: %d", __func__, slot_id, (int)result);
