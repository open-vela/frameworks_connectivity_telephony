...
int tapi_call_hanup_current_call_test(int slot_id)
{
    ...
    if (ret) {
        syslog(LOG_ERR, "tapi_call_hanup_current_call_test execute fail in %s, ret: %d",
            __func__, ret);
        res = -1;
        goto on_exit;
    }
    ...
    if (judge()) {
        syslog(LOG_DEBUG, "tapi_call_hanup_current_call_test is not executed in %s", __func__);
        res = -1;
        goto on_exit;
    }
...
int tapi_call_dial_using_phone_number_with_numerous_code_test(int slot_id)
{
...
int call_check_alerting_status(void)
{
    ...
    if (judge()) {
        syslog(LOG_DEBUG, "call_check_status is not executed in %s", __func__);
        res = -1;
        goto on_exit;
    }
...
int remote_operation_third_call_waiting_test(int slot_id)
{
    int res = 0;
    judge_data_init();
    test_case_data_init();
    ...
    remote_call_operation(slot_id, "10001", INCOMING_CALL);
...
int call_clear_voicecall_slot_set(void)
{
    ...
    if (ret) {
        syslog(LOG_ERR, "tapi_call_set_default_voicecall_slot_test execute fail, ret: %d", ret);
        res = -1;
        goto on_exit;
    }
...
int call_release_and_answer(int slot_id)
{
    ...
    sleep(3);
    if (remote_operation_third_call_waiting_test(slot_id) < 0) {
        syslog(LOG_ERR, "Waiting call fail in %s", __func__);
        res = -1;
        goto on_exit;
    }
    ...
    sleep(3);
    if (tapi_call_release_and_answer_test(slot_id) < 0) {
...
int outgoing_call_hold_and_unhold_by_caller(int slot_id)
{
    ...
    if (tapi_call_hold_test(slot_id) < 0) {
...
// todo: 56
int call_dial_second_call_and_hangup_by_caller_before_answering(int slot_id)
{
    int ret1 = tapi_call_listen_call_test(slot_id);
    judge_data_init();
    test_case_data_init();
